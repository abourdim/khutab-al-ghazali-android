#!/usr/bin/env bash
# ============================================================================
#  Khutab al-Ghazali Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/32-khutab-al-ghazali}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.khutabalghazali}"
AVD_NAME="${AVD_NAME:-khutab_al_ghazali_test}"
...
cmd_test() {
  ...
  local out="/tmp/khutab-al-ghazali-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Khutab al-Ghazali Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
